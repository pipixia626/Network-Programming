...
unsigned short _get_member_count(int sid);
int getval(int sid, int member);
void dipval(int sid, int member);
...
void opensem(int *sid, key_t key)
{
    ...
    {
        printf("Semaphore set does not exists\n");
        exit(1);
    }
}
void createsem(int *sid, key_t key, int members)
{
    ...
    if (members > SEMMSL)
    {
        printf("Sorry ,max out of semaphores in a set%d\n", SEMMSL);
        exit(1);
    }
    printf("Attemping to create new semaphores set with%d members\n", members);
